...
from . import micro_cluster, preprocessing
...
class DenStream:
    ...
    def _merging(self, current_time: int, feature_array: np.ndarray, label: Optional[int] = None, data_id: Optional[Any] = None) -> None:
        """
        ...
        if len(self.o_micro_clusters) > 0:
            ...
            closest_o_cluster.append(current_time, feature_array, label, data_id)
        ...
        new_o_cluster.append(current_time, feature_array, label, data_id)
    ...
    def partial_fit(
        ...
        :param time: The current time.
        :param label: Specifying the true label of a data point. None indicates that it is not provided.
        :param data_id: Identification for a given data point p.
        ...
        self._validate_fit_input(time, feature_array, label, data_id)

        self.iterations += 1

        self._merging(time, feature_array, label, data_id)
    ...
    def fit_generator(
        ...
        # WARNING: THIS FUNCTION WILL NOT WORK PROPERLY
        """
        ...
        while True:
            ...
            self.partial_fit(feature_array, time, label, data_id, request_period=request_period)
    ...
    def _cluster_evaluate(self, iteration: int) -> None:
        ...
        :param iteration: current iteration, i.e. number of data-points.
        :return
        """
    ...
    def _validate_fit_input(self, time: int, feature_array: np.ndarray, label: Optional[int] = None, data_id: Optional[Any] = None) -> None:
        """
        ...
        :param data_id: Unique id a given data point p. None indicates that the label is not provided.
        :return
        """
        ...
        if time < self.current_time:
            raise ValueError("received data from the past; data_time < current_time")
        else:
    ...
    def state_dict(self):
        ...
        del state_dict['label_metrics_list'] # do not export metrics functions, must setup manually?
        del state_dict['no_label_metrics_list'] # do not export metrics functions, must setup manually?
        del state_dict['metrics_results'] # do not export metrics results, must setup manually?

        state_dict['o_micro_clusters'] = [mc.state_dict() for mc in state_dict['o_micro_clusters']]
        state_dict['p_micro_clusters'] = [mc.state_dict() for mc in state_dict['p_micro_clusters']]
        state_dict['completed_o_clusters'] = [mc.state_dict() for mc in state_dict['completed_o_clusters']]
        state_dict['completed_p_clusters'] = [mc.state_dict() for mc in state_dict['completed_p_clusters']]

        state_dict['model'] = {'model_type':'DBSCAN',
    ...
    def load_state_dict(self,state_dict):
        ...
        def load_mc(state_dicts):
            ...
            for mc,state_dict in zip(mcs,state_dicts):
                mc.load_state_dict(state_dict)
            return mcs

        state_dict['o_micro_clusters'] = load_mc(state_dict['o_micro_clusters'])
        state_dict['p_micro_clusters'] = load_mc(state_dict['p_micro_clusters'])
